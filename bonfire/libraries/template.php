...
class Template
{
    ...
    protected static $templateConfig = null;
	 ...
	 * @return void
	 */
	public static function render($layout=NULL, $_renderToVar = false)
	{
		$output = '';
        $controller = false;
        if(!$_renderToVar){
    ...
    public static function renderToVar(){
    ...
    public static function getViewContent($view_name){

        self::load_view($view_name, NULL, self::$ci->router->class . '/' . self::$ci->router->method, FALSE, $output);
    ...
    public static function block_module($block_name='', $default_view='', $data=array(), $themed=FALSE)
    {
        return ;
        if (empty($block_name))
        {
        ...
        self::load_view($block_view_name, $data, FALSE, false, $output);
    ...
    public static function renderBlock($block_name='', $default_view='', $data=array(), $themed=FALSE)
    {
        if (empty($block_name))
        {
            self::debug_message('No block name provided.');
        ...
        $currentModuleName = self::$ci->load->getCurrentModuleName();
        $block_view_name = Blocks::getBlockView($currentModuleName,$block_name);
        if(!$block_view_name){
            return false;
        }
        self::load_view($block_view_name, $data, false, true, $output);
        $block_data = array('block' => $block_view_name, 'output' => $output);
        Events::trigger('after_block_render', $block_data);
...
EOF;
	 ...
	 */
	public  static function find_file($view=NULL, $data=NULL)
	{
		if (empty($view))
		{
			return FALSE;
